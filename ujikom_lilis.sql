...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 29 Mar 2022 pada 06.08
...
-- Prosedur
--
CREATE DEFINER=`root`@`localhost` PROCEDURE ` detailreservasi` ()  SELECT * FROM detailreservasi$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `extra` ()  SELECT *FROM extra$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `fasilitas` ()  SELECT * FROM fasilitas$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `idfasilitas` (IN `idf` INT(1))  SELECT * FROM fasilitas WHERE idfasilitas = idf$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `kamargalery` ()  SELECT * FROM kamargalery$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `namatamu` ()  SELECT * FROM tamu$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `users` ()  SELECT FORM users$$
...
(1, 9, 1, 1, 0, '2022-03-27 15:11:31', NULL);
...
(1, 'pesanan 1', 20000, '2022-03-27 15:09:45', NULL);
...
(8, 1, 'Standard Room 2', 250000, 3, '<p>Tersedia</p>\r\n', '2022-03-22 18:59:46', NULL);
...
(9, 7, 1, '2022-03-27', '0000-00-00', 1, 1, 'checkin', '2022-03-27 15:11:31', NULL, NULL, NULL, '0000-00-00 00:00:00', '0000-00-00 00:00:00');
...
CREATE TRIGGER `triger hotel` AFTER UPDATE ON `reservasi` FOR EACH ROW BEGIN
    IF new.status='checkin' THEN
        ...
        END IF;
         IF new.status='checkin' THEN
                UPDATE kamar SET
                jumlahqty=jumlahqty-old.qtykamar WHERE idkamar = old.kamarid;
          END IF;
END
$$
DELIMITER ;
...
(12, '131313', 'yadi', 'perempuan', 'INDONESIA', '085747920676', 'yadi@gmail.com', '123456789', 'YA', '2022-03-25 06:00:41', NULL);
...
(2, 'Lilis Suryani', 'perempuan', 'Purwosari RT09/04 Langensari Kota Banjar', '085747920676', 'lilis@admin.com', '$2y$10$yhJDSQtI/EgLbwV7t4Lha.593q/YLih1Tu9kJn/tXgGZRdlLdb4tS', 'Y', 'ADMIN', '2067-IMG-Picture.jpg', '2022-03-22 18:42:47', NULL),
(3, 'Lilis Suryani', 'perempuan', 'Indonesia', '085747920676', 'lilis@resepsionis.com', '$2y$10$EZaEAxdtZ.c3T/9q4B5bd.V245dawBPOgc6yboDtO8S3Mpg1ovgw2', 'Y', 'RECEPTIONIST', '6639-IMG-Picture.jpg', '2022-03-23 16:47:28', NULL);
...
--
ALTER TABLE `tamu`
  MODIFY `idtamu` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
...
--
ALTER TABLE `rating`
  ADD CONSTRAINT `rating_ibfk_1` FOREIGN KEY (`kamarid`) REFERENCES `kamar` (`idkamar`);
